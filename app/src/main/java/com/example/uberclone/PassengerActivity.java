...
public class PassengerActivity extends FragmentActivity implements OnMapReadyCallback, View.OnClickListener {
    ...
    public void onClick(View view) {
        ...
        if  (isUberCancelled == true){
            ...
            carRequestQuery.findInBackground(new FindCallback<ParseObject>() {
                @Override
                public void done(List<ParseObject> requestLists, ParseException e) {
                    if(requestLists.size() > 0 && e == null){
                        isUberCancelled = true;
                        btnRequestCar.setText("Request a Car");

                        for(ParseObject uberRequest : requestLists){
                                                ...
                                                if (ContextCompat.checkSelfPermission(PassengerActivity.this, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
                                                    ...
                                                    if (milesDistance < 0.3){
                                                        ...
                                                        requestObject.deleteInBackground(new DeleteCallback() {
                                                            ...
                                                            public void done(ParseException e) {
                                                                if (e == null){
                                                                    Toast.makeText(PassengerActivity.this, "Your Uber Has Arrived!", Toast.LENGTH_SHORT).show();
                                                                    isCarReady = false;
                                                                    isUberCancelled = true;
                                                                    btnRequestCar.setText("Book Another Ride");
                                                                }
                                                            }
                                                        });
                                                    }else {
                                                        ...
                                                        Marker markerDriver = mMap.addMarker(new MarkerOptions().position(dLocation).title("Driver Location"));
                                                        Marker markerPassenger = mMap.addMarker(new MarkerOptions().position(pLocation).title("Passenger Location"));

                                                        ArrayList<Marker> myMarkers = new ArrayList<>();
                                                        myMarkers.add(markerDriver);
                                                        myMarkers.add(markerPassenger);
